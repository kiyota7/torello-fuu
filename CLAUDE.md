...
- バックエンド(8080)・フロントエンド(5173)など、アプリ側で指定されているデフォルトポートで
  必ず起動すること。ポートが競合していても、空いている別のポートに逃がしてはいけない
- 詳細な手順(競合確認・プロセス停止・再起動・疎通確認)は`.claude/skills/start-dev-servers/SKILL.md`
  のSkillに従うこと
